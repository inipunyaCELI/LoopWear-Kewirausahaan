...
<div class="container checkout-page pt-4">
    ...
    <a href="<?php echo e(route('cart.index')); ?>" class="btn-back">&larr; Kembali ke Keranjang</a>

    <form id="formCheckout" action="<?php echo e(route('checkout.store')); ?>" method="POST">
        <?php echo csrf_field(); ?>
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    <textarea name="alamat" class="form-loop" rows="3"
                              ...
                              required><?php if(auth()->check() && auth()->user()->address): ?><?php echo e(auth()->user()->address); ?>, <?php echo e(auth()->user()->city); ?> <?php echo e(auth()->user()->postal_code); ?><?php endif; ?></textarea>
                </div>
            </div>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                <div style="width:50px; height:50px; background:#eee; border-radius:8px; overflow:hidden;">
                                    <img src="<?php echo e(asset('images/' . ($item['gambar'] ?? 'no-image.png'))); ?>"
                                         style="width:100%; height:100%; object-fit:cover;" alt="Gambar Produk">
                                </div>
                                <span class="fw-bold text-dark"><?php echo e($item['nama']); ?></span>
                            </div>
                        </td>
                    ...
                    <?php if($voucher): ?>
                        <div class="voucher-aktif-box">
                            <span class="fw-bold" style="color:#47510B; font-size:0.9rem;">
                                🏷️ <strong><?php echo e($voucher['kode']); ?></strong>
                                <?php if($voucher['tipe'] == 'persen'): ?>
                                    — diskon <?php echo e($voucher['nilai']); ?>%
                                <?php elseif($voucher['tipe'] == 'nominal'): ?>
                                    — potongan Rp <?php echo e(number_format($voucher['nilai'], 0, ',', '.')); ?>

                                <?php elseif($voucher['tipe'] == 'gratis_ongkir'): ?>
                                    — gratis ongkir
                                <?php endif; ?>
                            </span>
                        ...
                        <div class="voucher-box">
                            ...
                            <div class="d-flex gap-2">
                                <input type="text" name="kode" form="formVoucher" placeholder="Masukkan kode voucher"
                                    class="voucher-input" value="<?php echo e(old('kode')); ?>">
                                <button type="submit" form="formVoucher" class="btn-voucher">Pakai</button>
                            </div>                            
                            <?php if(session('voucher_error')): ?>
                                <p class="text-danger small mt-2 mb-0 fw-bold">⚠️ <?php echo e(session('voucher_error')); ?></p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
...
<?php if(!$voucher): ?>
<form id="formVoucher" action="<?php echo e(route('checkout.voucher')); ?>" method="POST" style="display: none;">
    <?php echo csrf_field(); ?>
    <?php $__currentLoopData = $selected; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <input type="hidden" name="selected[]" value="<?php echo e($id); ?>">
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\ASUS\OneDrive\Documents\GitHub\LoopWear-Kewirausahaan\kewirausahaan\loopwear\resources\views/checkout.blade.php ENDPATH**/ ?>